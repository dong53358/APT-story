...
const MyPosts = () => {
  ...
  const { documents: myPostsData, error } = useCollection("board", [
    "uid",
    "==",
    user.uid,
  ]);
  ...
  const imgMatch = useMatch(`/mypage/my-posts/img/:Id`);
  const navigate = useNavigate();
  const { scrollY } = useScroll();

  const imgClick = (imgId, imgUrl) => {
    setImgUrl(imgUrl);
    navigate(`/mypage/my-posts/img/${imgId}`);
